...
ns.widgets.image.prototype.appendTo = function ($wrapper) {
    ...
    var htmlString =
        '<div class="file"></div>' + '<div class="h5p-editor-image-buttons">';

    if (!this.field.disableCopyright) {
        htmlString +=
            '<button class="h5peditor-button-textual h5p-copyright-button">' +
    ...
    $container
        .find('.h5p-copyright-button')
        .add($dialog.find('.h5p-close'))
        .click(function () {
...
ns.widgets.image.prototype.addFile = function () {
    ...
    if (this.params === undefined) {
        ...
        html +=
            ...
            ns.t('core', 'addFile') +
            '">' +
            '<div class="h5peditor-field-file-upload-text">' +
            ns.t('core', 'add') +
            '</div>' +
            '</a>';
    ...
    var fileHtmlString =
        ...
        this.id +
        '" title="' +
        ns.t('core', 'changeFile') +
        '" class="thumbnail"';
    if (this.field.description !== undefined) {
        fileHtmlString +=
            ' aria-describedby="' + ns.getDescriptionId(this.id) + '"';
    }
    fileHtmlString +=
        '><img alt="' +
        altText +
