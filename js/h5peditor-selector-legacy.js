...
ns.SelectorLegacy = function (libraries, selectedLibrary, changeLibraryDialog) {
    ...
    var defaultLibraryParameterized = selectedLibrary
        ? selectedLibrary.replace('.', '-').toLowerCase()
        : undefined;
    ...
    for (var i = 0; i < libraries.length; i++) {
        ...
        if (
            ...
            if (
                ...
                library.name === defaultLibraryParameterized
            ) {
            ...
            options +=
                '>' +
                library.title +
                (library.isOld === true ? ' (deprecated)' : '') +
                '</option>';
        }
    }

    this.$selector = ns.$('').change(function () {
 ...
 */
ns.SelectorLegacy.prototype.resetSelection = function (
    library,
    params,
    metadata
) {
