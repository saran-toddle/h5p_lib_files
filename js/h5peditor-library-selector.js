...
ns.LibrarySelector = function (libraries, defaultLibrary, defaultParams) {
    ...
    this.defaultLibraryParameterized = defaultLibrary
        ? defaultLibrary.replace('.', '-').toLowerCase()
        : undefined;
    ...
    this.selector = new ns.SelectorLegacy(
        libraries,
        defaultLibrary,
        changeLibraryDialog
    );
    ...
    var librarySelectHandler = function (library) {
        ...
        that.loadSemantics(
            library.uberName,
            that.selector.getParams(),
            that.selector.getMetadata()
        );

        that.$tutorialUrl
            .attr('href', library.tutorialUrl ? library.tutorialUrl : '#')
            .toggle(!!library.tutorialUrl);
        that.$exampleUrl
            .attr('href', library.exampleUrl ? library.exampleUrl : '#')
    ...
    H5P.externalDispatcher.on(
        'datainclipboard',
        this.updateCopyPasteButtons.bind(this)
    );
...
ns.LibrarySelector.prototype.appendTo = function ($element) {
    ...
    //     this.$copyButton = $buttons
    //         .find('.h5peditor-copy-button')
    //         .click(function () {
    //             H5P.clipboardify({
    ...
    //             );
    //         });
    //     this.$pasteButton = $buttons
    //         .find('.h5peditor-paste-button')
...
ns.LibrarySelector.prototype.updateCopyPasteButtons = function () {
    ...
    // this.$copyButton.prop('disabled', false).toggleClass('disabled', false);
    ...
    //         'title',
    //         canPaste
    //             ? ns.t('core', 'pasteAndReplaceFromClipboard')
    //             : pasteCheck.description
    //     )
    //     .toggleClass('disabled', !canPaste)
    //     .prop('disabled', !canPaste);

    // this.selector.setCanPaste &&
    //     this.selector.setCanPaste(
    //         canPaste,
    //         !canPaste ? pasteCheck.description : undefined
    //     );
};
...
ns.LibrarySelector.prototype.pasteContent = function () {
    ...
    ns.confirmReplace(
        self.getCurrentLibrary(),
        self.$parent.offset().top,
        function () {
            self.selector.resetSelection(
                clipboard.generic.library,
                clipboard.generic.params,
                clipboard.generic.metadata,
                false
            );
            self.setLibrary();
        }
 ...
 * @returns {unresolved}
 */
ns.LibrarySelector.prototype.loadSemantics = function (
    library,
    params,
    metadata
) {
    ...
    this.$parent.attr(
        'class',
        'h5peditor ' +
            library.split(' ')[0].toLowerCase().replace('.', '-') +
            '-editor'
    );
    ...
    var $loading = ns
        .$(
            '<div class="h5peditor-loading h5p-throbber">' +
                ns.t('core', 'loading') +
                '</div>'
        )
    ...
    ns.loadLibrary(library, function (semantics) {
        if (!semantics) {
            ...
            if (params) {
                ...
                library === that.defaultLibrary ||
                library === that.defaultLibraryParameterized
            ) {
            ...
            const defaultLanguage =
                metadata && metadata.defaultLanguage
