        ...
        self.upload = function (file, filename) {
            ...
            request.onload = function () {
                ...
                try {
                    ...
                    if (uploadUrl) {
                        imageRequest.open('PUT', uploadUrl, true);
                        imageRequest.send(file);
                        imageRequest.upload.onprogress = function (e) {
                            if (e.lengthComputable) {
                                self.trigger(
                                    'uploadProgress',
                                    e.loaded / e.total
            ...
            request.open(
                'POST',
                'https://eu-west-1-production-apis.toddleapp.com/auth/generateS3SignedURL',
                true
            );
            ...
            request.send('fileMimeType=image%2Fpng');
            self.trigger('upload');
        };
