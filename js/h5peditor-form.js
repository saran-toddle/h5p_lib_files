...
ns.Form = function (library, startLanguages, defaultLanguage) {
    ...
    this.$form = ns.$(
        '' +
            ...
            '<label class="language-label" for="h5peditor-language-switcher">' +
            ns.t('core', 'language') +
            ':</label>' +
            '<select id="h5peditor-language-switcher">' +
            '<option value="-">' +
            ns.t('core', 'noLanguagesSupported') +
            '</option>' +
            '</select>' +
            '</div>' +
    ...
    if (ns.FullscreenBar !== undefined) {
        ...
        if (
            library.indexOf('H5P.CoursePresentation') === -1 &&
    ...
    const createOptions = function () {
        ...
        for (let code in languages) {
            let label = ns.supportedLanguages[code]
                ? ns.supportedLanguages[code]
                : code.toLocaleUpperCase();
            options +=
                '<option value="' +
                code +
                '"' +
                (code === ns.defaultLanguage ? ' selected' : '') +
                '>' +
                label +
                '</option>';
        }
    ...
    const updateCommonFields = function (lang) {
        ...
        for (let lib in ns.libraryCache) {
            ...
            if (
                ns.renderableCommonFields[lib] &&
                ns.renderableCommonFields[lib].fields
            ) {
                for (
                    let j = 0;
                    j < ns.renderableCommonFields[lib].fields.length;
                    j++
                ) {
                    ...
                    const fieldTranslation = findFieldDefaultTranslation(
                        field.field,
                        ns.libraryCache[lib].semantics,
                        translation
                    );
            ...
            if (ns.libraryCache[lib].translation[lang] !== undefined) {
                ...
                ns.updateCommonFieldsDefault(
                    ns.libraryCache[lib].semantics,
                    ns.libraryCache[lib].translation[lang]
                );
            }
        }
    };
     ...
     */
    const findFieldDefaultTranslation = function (
        field,
        semantics,
        translation
    ) {
        for (let i = 0; i < semantics.length; i++) {
            ...
            if (
                ...
                const found1 = findFieldDefaultTranslation(
                    field,
                    semantics[i].fields,
            ...
            if (
                ...
                const found2 = findFieldDefaultTranslation(
                    field,
                    [semantics[i].field],
                    [translation[i].field]
                );
    ...
    const getDefaultValue = function (translation, field) {
        ...
        if (translation.fields !== undefined && translation.fields.length) {
            ...
            for (let i = 0; i < translation.fields.length; i++) {
                values[field.fields[i].name] = getDefaultValue(
    ...
    const loadTranslations = function (lang, done) {
        ...
        for (let li in ns.libraryCache) {
            if (
                ns.libraryCache[li] === 0 ||
                ns.libraryCache[li].translation[lang] === undefined
            ) {
                loadLibs.push(li);
            }
        }
            ...
            ns.$.post(
                ...
                function (res) {
                    for (let lib in res.data) {
                        ns.libraryCache[lib].translation[lang] = JSON.parse(
                            res.data[lib]
                        ).semantics;
                    }
                    done();
    ...
    $switcher.change(function (e) {
        ...
        const confirmDialog = new H5P.ConfirmationDialog({
            headerText: ns.t('core', 'changeLanguage', {
                ':language': ns.supportedLanguages[this.value]
                    ? ns.supportedLanguages[this.value]
        ...
        confirmDialog.on('confirmed', function () {
            ...
            const humanLang = ns.supportedLanguages[lang]
                ? ns.supportedLanguages[lang]
            ...
            if (!isSupportedByAll(lang)) {
                ...
                $notice.children('.first').html(
                    ns.t('core', 'notAllTextsChanged', {
                        ':language': humanLang
                    })
                );
                $notice.children('.last').html(
                    ns.t('core', 'contributeTranslations', {
                        ':language': humanLang,
                        ':url': 'https://h5p.org/contributing#translating'
                    })
                );
 ...
 * @returns {undefined}
 */
ns.Form.prototype.processSemantics = function (
    semantics,
    defaultParams,
    metadata
) {
    ...
    if (ns.enableMetadata(this.currentLibrary)) {
        this.metadataForm = new ns.MetadataForm(
            this,
            this.metadata,
            this.$form.children('.tree'),
            true
        );
    } else {
        this.metadataForm = H5PEditor.MetadataForm.createLegacyForm(
            this.metadata,
            this.$form.children('.tree')
        );
        ...
        switch (this.currentLibrary.split(' ')[0]) {
            ...
            case 'H5P.ImageHotspotQuestion':
                this.metadataForm
                    .getExtraTitleField()
                    .$item.css('padding', '20px 20px 0 20px');
                break;

            case 'H5P.CoursePresentation':
                this.metadataForm
                    .getExtraTitleField()
                    .$item.css('padding-bottom', '1em');
                break;
        }
    }
