...
ns.libraryRequested = function (libraryName, callback) {
    ...
    if (!ns.libraryLoaded[libraryName]) {
        ...
        var loadingJs = false;
        if (
            libraryData.javascript !== undefined &&
            libraryData.javascript.length
        ) {
            libraryData.javascript.forEach(function (path) {
                if (!H5P.jsLoaded(path)) {
                    ...
                    ns.loadJs(path, function (err) {
                        ...
                        var isFinishedLoading = libraryData.javascript.reduce(
                            function (hasLoaded, jsPath) {
                        ...
                        if (isFinishedLoading) {
                            ...
                            if (libraryData.translations) {
                                for (var machineName in libraryData.translations) {
                                    H5PEditor.language[machineName] =
                                        libraryData.translations[machineName];
                                }
                            }
            ...
            ns.$.ajax({
                ...
                success: function (libraryData) {
                    ...
                    if (libraryData.defaultLanguage !== null) {
                        libraryData.translation[defaultLanguage] = JSON.parse(
                            libraryData.defaultLanguage
                        ).semantics;
                        delete libraryData.defaultLanguage; // Avoid caching a lot of unused data
                        ns.updateCommonFieldsDefault(
                            semantics,
                            libraryData.translation[defaultLanguage]
                        );
                    }
                    ...
                    ns.libraryRequested(libraryName, function (semantics) {
                        ...
                        if (ns.loadedCallbacks[libraryName]) {
                            for (
                                var i = 0;
                                i < ns.loadedCallbacks[libraryName].length;
                                i++
                            ) {
 ...
 */
ns.updateCommonFieldsDefault = function (
    semantics,
    translation,
    parentIsCommon
) {
    for (let i = 0; i < semantics.length; i++) {
        ...
        if (
            isCommon &&
            semantics[i].default !== undefined &&
            translation[i] !== undefined &&
            translation[i].default !== undefined
        ) {
            // Update value
            semantics[i].default = translation[i].default;
        }
        if (
            semantics[i].fields !== undefined &&
            semantics[i].fields.length &&
            translation[i].fields !== undefined &&
            translation[i].fields.length
        ) {
            // Look into sub fields
            ns.updateCommonFieldsDefault(
                semantics[i].fields,
                translation[i].fields,
                isCommon
            );
        }
        if (
            semantics[i].field !== undefined &&
...
ns.renderCommonField = function (machineName, libraries) {
    ...
    if (renderableCommonFields.length) {
        var libraryName =
            machineName === ns.constants.otherLibraries
                ? machineName
                : machineName.length
                ? machineName.split(' ')[0]
                : '';
        ...
        var hasLibraryWrapper =
            !!ns.renderableCommonFields[machineName].wrapper;
        var commonFieldsLibraryWrapper =
            ns.renderableCommonFields[machineName].wrapper;
        if (!hasLibraryWrapper) {
            commonFieldsLibraryWrapper = document.createElement('fieldset');
            var libraryWrapperClass = libraryName
                .replace(/\s+/g, '-')
                .toLowerCase();
 ...
 * @returns {undefined}
 */
ns.processSemanticsChunk = function (
    semanticsChunk,
    params,
    $wrapper,
    parent,
    ...
    if (parent.passReadies === undefined) {
        throw (
            'Widget tried to run processSemanticsChunk without handling ready callbacks. [field:' +
            parent.field.type +
            ':' +
            parent.field.name +
            ']'
    ...
    for (var i = 0; i < semanticsChunk.length; i++) {
        ...
        if (field.name === undefined) {
            throw ns.t('core', 'missingProperty', {
                ':index': i,
                ':property': 'name'
            });
        }
        ...
        if (field.common !== undefined && field.common) {
            ...
            ns.renderableCommonFields[library] =
                ns.renderableCommonFields[library] || {};
            ns.renderableCommonFields[library].fields =
                ns.renderableCommonFields[library].fields || [];
        ...
        var fieldInstance = new ns.widgets[widget](
            parent,
            field,
            params[field.name],
...
ns.addCommonField = function (field, parent, params, ancestor, skipAppendTo) {
    ...
    if (
        ancestor.commonFields[parent.currentLibrary][field.name] === undefined
    ) {
        var widget = ns.getWidgetName(field);
        ancestor.commonFields[parent.currentLibrary][field.name] = {
            instance: new ns.widgets[widget](
                parent,
                field,
                params[field.name],
                function (field, value) {
...
ns.removeChildren = function (children) {
    ...
    for (var i = 0; i < children.length; i++) {
        ...
        var isCommonField =
            children[i].field === undefined ||
            children[i].field.common === undefined ||
            !children[i].field.common;

        var hasRemove =
            children[i].remove instanceof Function ||
...
ns.findSemanticsField = function (fieldName, semanticsStructure) {
    if (Array.isArray(semanticsStructure)) {
        for (let i = 0; i < semanticsStructure.length; i++) {
            var semanticsField = ns.findSemanticsField(
                fieldName,
                semanticsStructure[i]
            );
...
ns.createFieldMarkup = function (field, content, inputId) {
    content = content || '';
    var markup =
        this.createLabel(field, '', inputId) +
...
ns.wrapFieldMarkup = function (field, markup) {
    ...
    var wrapperClasses = this.joinNonEmptyStrings([
        'field',
        'field-name-' + field.name,
        field.type,
        ns.createImportance(field.importance),
        field.widget
    ]);
...
ns.createLabel = function (field, content, inputId) {
    ...
    if (
        field.widget === 'coursepresentation' &&
    ...
    if (field.label !== 0) {
        html +=
            ...
            (field.optional ? '' : ' h5peditor-required') +
            '">' +
            (field.label === undefined ? field.name : field.label) +
            '</span>';
    }
...
ns.createImportantDescription = function (importantDescription) {
    ...
    if (importantDescription !== undefined) {
        html +=
            '<div class="h5peditor-field-important-description">' +
            '<div class="important-description-tail">' +
            '</div>' +
            '<div class="important-description-close" role="button" tabindex="0" aria-label="' +
            ns.t('core', 'hideImportantInstructions') +
            '">' +
            '<span>' +
            ns.t('core', 'hide') +
            '</span>' +
        ...
        if (importantDescription.description !== undefined) {
            html +=
                '<div class="important-description-content">' +
        ...
        if (importantDescription.example !== undefined) {
            html +=
                '<div class="important-description-example">' +
...
ns.bindImportantDescriptionEvents = function (widget, fieldName, parent) {
    ...
    if (librarySelector.currentLibrary !== undefined) {
        ...
        context =
            (lib + '-' + fieldName).replace(/\.|_/g, '-') +
            '-important-description-open';
    }
    ...
    widget.$item
        .find('.important-description-show')
        .click(function () {
    ...
    widget.$item
        .find('.important-description-close')
        .click(function () {
...
ns.createButton = function (id, title, handler, displayTitle) {
    var options = {
        class:
            'h5peditor-button ' +
            (displayTitle ? 'h5peditor-button-textual ' : '') +
            id,
        role: 'button',
        tabIndex: 0,
        'aria-disabled': 'false',
...
ns.enableMetadata = function (library) {
    ...
    if (
        ...
        (library.majorVersion === block.majorVersion &&
            library.minorVersion > block.minorVersion)
    ) {
        return true;
    }
...
ns.canPastePlus = function (clipboard, libs) {
    ...
    for (let i = 0; i < candidates.length; i++) {
        if (
            candidates[i].majorVersion + '.' + candidates[i].minorVersion ===
            versionClip
        ) {
...
ns.ContentType.getPossibleUpgrade = function (library, libraries) {
    ...
    for (let i = 0; i < libraries.length; i++) {
        ...
        if (
            candiate.installed !== false &&
            ns.ContentType.hasSameName(candiate, library) &&
            ns.ContentType.isHigherVersion(candiate, library)
        ) {
            // Check if the upgrade is better than the previous upgrade we found
            if (
                !possibleUpgrade ||
                ns.ContentType.isHigherVersion(candiate, possibleUpgrade)
            ) {
...
ns.ContentType.isHigherVersion = function (candiate, original) {
    return (
        ns.ContentType.getMajorVersion(candiate) >
            ns.ContentType.getMajorVersion(original) ||
        (ns.ContentType.getMajorVersion(candiate) ==
            ns.ContentType.getMajorVersion(original) &&
            ns.ContentType.getMinorVersion(candiate) >
                ns.ContentType.getMinorVersion(original))
    );
};
...
ns.ContentType.hasSameName = function (candiate, original) {
    return (
        ns.ContentType.getName(candiate) === ns.ContentType.getName(original)
    );
};
...
ns.ContentType.getNameVersionString = function (library) {
    return (
        ns.ContentType.getName(library) +
        ' ' +
        ns.ContentType.getMajorVersion(library) +
        '.' +
        ns.ContentType.getMinorVersion(library)
    );
};
...
ns.ContentType.getMajorVersion = function (library) {
    return parseInt(
        library.localMajorVersion !== undefined
            ? library.localMajorVersion
...
ns.ContentType.getMinorVersion = function (library) {
    return parseInt(
        library.localMinorVersion !== undefined
            ? library.localMinorVersion
...
ns.ContentType.getName = function (library) {
    return library.machineName !== undefined
        ? library.machineName
        : library.name;
};
    ...
    return function contentUpgrade(fromLibrary, toLibrary, parameters, done) {
        ns.loadJs(
            H5PIntegration.libraryUrl +
                '/h5p-version.js' +
                H5PIntegration.pluginCacheBuster,
            function (err) {
                ns.loadJs(
                    H5PIntegration.libraryUrl +
                    ...
                    function (err) {
                        ...
                        new H5P.ContentUpgradeProcess(
                            ns.ContentType.getName(fromLibrary),
                            new H5P.Version(fromLibrary),
                            new H5P.Version(toLibrary),
                            JSON.stringify(parameters),
                            1,
                            function (name, version, next) {
                                loadLibrary(
                                    name,
                                    version,
                                    function (err, library) {
                                        if (library.upgradesScript) {
                                            ns.loadJs(
                                                library.upgradesScript,
                                                function (err) {
                                                    if (err) {
                                                        err =
                                                            'Error loading upgrades ' +
                                                            name +
                                                            ' ' +
                                                            version;
                                                    }
                                ...
                                if (err) {
                                    ...
                                    switch (err.type) {
                                        ...
                                        case 'scriptMissing':
                                            message +=
                                                ': ' +
                                                ns.t('core', 'scriptMissing', {
                                                    '%lib': err.library
                                                });
                                            break;
                                    }

                                    var confirmErrorDialog =
                                        new H5P.ConfirmationDialog({
                                            headerText: header,
                                            dialogText: message,
...
ns.supportedLanguages = {
    aa: 'Afar',
    ab: 'Abkhazian (Ð°Ò§ÑÑƒÐ° Ð±Ñ‹Ð·ÑˆÓ™Ð°)',
    ae: 'Avestan',
    af: 'Afrikaans',
    ak: 'Akan',
    ...
    'pt-pt': 'Portuguese, Portugal (PortuguÃªs)',
